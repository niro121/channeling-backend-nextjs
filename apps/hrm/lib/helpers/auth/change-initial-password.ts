...
export async function changeInitialPassword(
  ...
  const confirmPassword =
    typeof input.confirmPassword === 'string' ? input.confirmPassword.trim() : undefined;
  ...
  if (user.mustChangePassword !== true) {
    return { success: false, status: 400, error: 'This account does not require a password change' };
  }
