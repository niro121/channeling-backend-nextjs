  ...
  const handleSelectMethod = async (methodId: string, clearTwoFactorCode: () => void) => {
    ...
    try {
      const res = await fetch('/api/auth/request-2fa-code', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: pending2FA.email, password: pending2FA.password, method: methodId }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) { toast({ variant: 'destructive', title: 'Error', description: data?.error ?? 'Could not send code.' }); return; }
      setPending2FA((prev) =>
        prev ? { ...prev, selectedMethod: methodId, twoFactorToken: data.twoFactorToken ?? null, message: data.message ?? 'Enter your verification code below.', needsSetup: !!data.needsSetup, uri: data.uri, secret: data.secret } : null
      );
    } finally {
  ...
  return (
    <>
      ...
      <div className="lg:hidden mb-6 text-center">
        <span className="text-xl font-semibold text-foreground">{process.env.NEXT_PUBLIC_BRAND_NAME || 'Ruhunu'} HRM</span>
      </div>
                      ...
                      <>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Button type="button" variant="outline" size="icon" onClick={() => { if (pending2FA.secret) { navigator.clipboard.writeText(pending2FA.secret); setCopied(true); setTimeout(() => setCopied(false), 2000); } }} aria-label="Copy secret">
