...
import { getAttendanceLogHistoryAction } from '@/app/actions/attendance-actions/attendance-log.actions';
import type {
  AttendanceLogHistoryEntry,
  AttendanceLogRecord
} from '@/types/attendance';

type SheetLogHistoryProps = {
  open: boolean;
  record: AttendanceLogRecord | null;
  onOpenChange: (open: boolean) => void;
};

export default function SheetLogHistory({
  open,
  record,
  onOpenChange
}: SheetLogHistoryProps) {
  const [entries, setEntries] = useState<AttendanceLogHistoryEntry[]>([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (!open || !record?.staffId || !record.attendanceDate) {
      setEntries([]);
      return;
    }
    ...
    let cancelled = false;
    setLoading(true);
    void getAttendanceLogHistoryAction({
  ...
  }, [open, record?.staffId, record?.attendanceDate]);
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        <SheetHeader className="shrink-0 space-y-1 border-b border-border bg-background px-6 py-4 pr-14 text-left">
          ...
          <SheetDescription>
            {record
              ? `Audit trail for ${record.staffName} on ${record.attendanceDateLabel}.`
              : 'Audit trail for this attendance day.'}
          </SheetDescription>
        </SheetHeader>
