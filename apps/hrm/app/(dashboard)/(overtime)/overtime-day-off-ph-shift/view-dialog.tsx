...
type ExtraShiftViewDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  record: ExtraShiftRecord | null;
};
...
export function ExtraShiftViewDialog({
  open,
  onOpenChange,
  record
}: ExtraShiftViewDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-lg overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Day Off / PH shift details</DialogTitle>
        </DialogHeader>

        {record ? (
          <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <DetailItem label="ID">{record.formNumber}</DetailItem>
            <DetailItem label="Type">{record.shiftType}</DetailItem>
