...
const EXTRA_TIME_LINKS = [
  { id: 'fingerprint', label: 'Finger Print', icon: Fingerprint },
  { id: 'print', label: 'Print', icon: Printer },
  { id: 'analysis', label: 'Analysis', icon: TrendingUp }
] as const;

type ExtraTimeLinksCardProps = {
  className?: string;
  /** Horizontal chip row on smaller screens; vertical list on 2xl. */
  orientation?: 'responsive' | 'vertical' | 'horizontal';
};

export function ExtraTimeLinksCard({
  className,
  orientation = 'responsive'
}: ExtraTimeLinksCardProps) {
  ...
  return (
    ...
    >
      ...
      <CardContent>
        <nav aria-label="Extra time links" className={listClassName}>
          {EXTRA_TIME_LINKS.map((item) => {
