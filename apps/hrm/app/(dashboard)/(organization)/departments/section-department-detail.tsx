...
import { INSTITUTION_OPTIONS, getInstitutionName } from '@/types/institution';
import {
...
const statusOptions = DEPARTMENT_STATUS_OPTIONS.map((opt) => ({
  id: opt.id,
  name: opt.name
}));
...
export default function SectionDepartmentDetail() {
  ...
  const initialValues = useMemo<DepartmentFormValues>(() => {
    if (isNew || !selectedRecord) return emptyDepartmentFormValues();
    return {
      name: selectedRecord.name,
      description: selectedRecord.description,
      institution: String(selectedRecord.institution),
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try {
      const result = await deleteDepartmentAction(selectedRecord.id, {
        syncToChanneling: true
      });
      if (result.isError) {
        toast({
      ...
      const warning = result.data?.channelingWarning;
      setDeleteOpen(false);
      setSelectedId(null);
      setIsNew(false);
      toast({
        title: 'Deleted',
        description: warning ?? 'Department removed.'
      });
      router.refresh();
    } finally {
      setSaving(false);
    ...
    >
      ...
      {showEmptyState ? (
        <div className="flex flex-1 items-center justify-center p-8 text-center text-sm text-muted-foreground">
          Select a department from the list or click Add to create one.
        </div>
      ) : (
        <Formik
          key={formKey}
        ...
        >
          {(formik) => {
            const institutionLabel = getInstitutionName(
              formik.values.institution === ''
                ? null
                : Number.parseInt(formik.values.institution, 10)
            );
      ...
      <CustomAlertDialog
        ...
        description={
          selectedRecord
            ? `Remove "${selectedRecord.name}" from HRM${
