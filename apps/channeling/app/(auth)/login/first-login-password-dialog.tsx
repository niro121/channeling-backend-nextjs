...
export function FirstLoginPasswordDialog({
  ...
  const handleChangePassword = async () => {
    ...
    if (newP.length < MIN_PASSWORD_LENGTH || !PASSWORD_REGEX.test(newP)) {
      setError(
        `Password must be at least ${MIN_PASSWORD_LENGTH} characters long and include uppercase, lowercase, number, special character, and no spaces.`
      );
      return;
    }
    if (newP !== conf) {
  ...
  return (
    <CustomContentAlertDialog
      ...
      description="You must set a new password before you can sign in. Enter the password you were given, then choose a new one."
