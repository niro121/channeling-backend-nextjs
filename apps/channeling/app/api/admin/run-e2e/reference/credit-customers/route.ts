...
export async function GET() {
  ...
  try {
    const result = await getAllCreditCustomersOptionsService();
    if (!result.success || !result.data) {
      return NextResponse.json(
        { error: result.error?.message ?? "Failed to load credit customers" },
        { status: 500 }
      );
    }
    return NextResponse.json(result.data);
  } catch (e) {
    console.error("GET /api/admin/run-e2e/reference/credit-customers error:", e);
    return NextResponse.json(
      { error: e instanceof Error ? e.message : "Failed to load credit customers" },
      { status: 500 }
    );
  }
}
