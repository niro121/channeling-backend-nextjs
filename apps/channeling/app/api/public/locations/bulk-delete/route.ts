...
import { bulkDeletePublicLocations } from "@/services/public/location.service"
...
/**
 * POST /api/public/locations/bulk-delete
 * Body: { ids: string[] }
...
export async function POST(request: NextRequest) {
      ...
      NextResponse.json(
        {
          ...
          error_description: "ids must be a non-empty array of location ids",
        },
        { status: 400 }
      )
    )
  ...
  const result = await bulkDeletePublicLocations(ids)
    ...
    if (result.linked) {
      return withCors(
        NextResponse.json(
          {
            error: "conflict",
            error_description: result.error?.message ?? "Locations cannot be deleted",
          },
          { status: 409 }
        )
      )
      ...
      NextResponse.json(
        {
          ...
          error_description: result.error?.message ?? "Failed to delete locations",
        },
        { status }
      )
    )
