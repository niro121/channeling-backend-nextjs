...
import { bulkDeletePublicZones } from "@/services/public/zone.service"
...
/**
 * POST /api/public/zones/bulk-delete
 * Body: { ids: string[] }
...
export async function POST(request: NextRequest) {
      ...
      NextResponse.json(
        {
          ...
          error_description: "ids must be a non-empty array of zone ids",
        },
        { status: 400 }
      )
    )
  ...
  const result = await bulkDeletePublicZones(ids)
    ...
    if (result.linked) {
      return withCors(
        NextResponse.json(
          {
            error: "conflict",
            error_description: result.error?.message ?? "Zones cannot be deleted",
          },
          { status: 409 }
        )
      )
      ...
      NextResponse.json(
        {
          ...
          error_description: result.error?.message ?? "Failed to delete zones",
        },
        { status }
      )
    )
