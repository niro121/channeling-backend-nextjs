...
/**
 * Agent Detail Report — PDF ONLY (A4 portrait, matches Print).
 */
...
function drawTotalsBox(
  ...
  opts: {
    ...
    y: number;
    width: number;
    totals: { allowed: number; max: number; standard: number; balance: number };
  }
): number {
  const { x, y, width, totals } = opts;
  const titleH = 4.2;
  const bodyPad = 1.2;
  const itemH = 7;
  const boxH = titleH + bodyPad * 2 + itemH;

  doc.setFillColor(236, 236, 236);
  doc.setDrawColor(0, 0, 0);
  doc.setLineWidth(0.3);
  doc.rect(x, y, width, titleH, 'FD');
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(7);
  doc.setTextColor(0, 0, 0);
  doc.text('TOTAL', x + 1.2, y + 2.9);
  ...
  const bodyY = y + titleH + bodyPad;
  items.forEach((item, i) => {
    const cx = x + colW * i + 1;
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(5.5);
    doc.setTextColor(68, 68, 68);
    doc.text(item.label, cx, bodyY + 2);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(7);
    doc.setTextColor(0, 0, 0);
    doc.text(item.value, cx, bodyY + 5.2);
  });

  return boxH + 1.4;
}

export type DownloadAgentDetailPdfOptions = {
  reportName: string;
  summaryItems: BrandedPdfSummaryItem[];
  generatedAt: string;
...
export async function downloadAgentDetailReportPdf({
  reportName,
  summaryItems,
  generatedAt,
  rows,
  fileName = 'agent-detail-report.pdf',
}: DownloadAgentDetailPdfOptions): Promise<void> {
  const margin = 7;
  ...
  AGENT_DETAIL_PDF_COL_PERCENTS.forEach((pct, i) => {
    columnStyles[i] = { cellWidth: (contentWidth * pct) / 100 };
  });

  autoTable(doc, {
    head: [Array.from(AGENT_DETAIL_PDF_HEADERS)],
    body: compactRows.map((row) => agentDetailPdfCompactRow(row)),
    startY: y,
