...
/**
 * Agent Detail Report — Excel ONLY (A4 portrait, matches Print).
 */
...
const COL_COUNT = 6;
const COLUMN_WIDTHS = [12, 18, 11, 28, 22, 24];
...
export type DownloadAgentDetailExcelOptions = {
  reportName: string;
  summaryItems: BrandedPdfSummaryItem[];
  generatedAt: string;
  rows: ExportAgentDetailData[];
  fileName?: string;
  sheetName?: string;
};

export async function downloadAgentDetailReportExcel({
  reportName,
  summaryItems,
  generatedAt,
  rows,
  fileName = 'agent-detail-report.xlsx',
  sheetName = 'Agent Detail',
}: DownloadAgentDetailExcelOptions): Promise<void> {
  ...
  const compactRows = rows.map(mapAgentDetailCompactFromExportRow);
  const totals = sumAgentDetailCreditTotals(compactRows);
  const headers = Array.from(AGENT_DETAIL_PDF_HEADERS);
  ...
  for (const compact of compactRows) {
    const values = agentDetailPdfCompactRow(compact);
    for (let c = 0; c < values.length; c++) {
      const cell = sheet.getCell(row, c + 1);
      cell.value = values[c];
      cell.font = { size: 8, name: 'Arial', bold: c === 1 || c === 2 };
      cell.border = {
  ...
  totalsTitle.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFECECEC' } };
  totalsTitle.border = thinBorder;
  ...
  for (let i = 0; i < totalItems.length; i++) {
    ...
    if (endCol > startCol) {
      sheet.mergeCells(totalsLabelRow, startCol, totalsLabelRow, endCol);
      sheet.mergeCells(totalsValueRow, startCol, totalsValueRow, endCol);
    }
    const labelCell = sheet.getCell(totalsLabelRow, startCol);
    labelCell.value = totalItems[i]!.label;
    ...
    for (let c = startCol; c <= endCol; c++) {
      sheet.getCell(totalsLabelRow, c).border = thinBorder;
      sheet.getCell(totalsValueRow, c).border = thinBorder;
    }
    startCol = endCol + 1;
  }
  sheet.getRow(totalsLabelRow).height = 16;
  sheet.getRow(totalsValueRow).height = 18;
  row = totalsValueRow + 2;
