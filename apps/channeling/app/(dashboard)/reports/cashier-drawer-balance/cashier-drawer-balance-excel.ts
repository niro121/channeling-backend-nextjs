...
/**
 * Cashier Drawer Balance — Excel ONLY (A4 portrait, matches Print / PDF).
 * Flat columns: Till | Cashier | Cash | Card | Credit | Slip | Cheque | E-Wallet | Total
 */
...
import type { CashierDrawerBalanceReportExportRow } from '@/types/reports/cashier-drawer-balance';
...
const KEYS: (keyof CashierDrawerBalanceReportExportRow)[] = [
  'till',
  'cashier',
  'cash',
  'card',
...
/** Proportional to print: Till/Cashier wider, amount columns even. */
const COLUMN_WIDTHS = [28, 20, 11, 11, 11, 11, 11, 11, 12];
...
export type DownloadCashierDrawerBalanceExcelOptions = {
  ...
  generatedAt: string;
  rows: CashierDrawerBalanceReportExportRow[];
  fileName?: string;
  sheetName?: string;
};

export async function downloadCashierDrawerBalanceReportExcel({
  reportName,
  summaryItems,
  generatedAt,
  rows,
  fileName = 'cashier-drawer-balance.xlsx',
  sheetName = 'Drawer Balance',
}: DownloadCashierDrawerBalanceExcelOptions): Promise<void> {
  ...
  for (const dataRow of rows) {
    totals.cash += parseAmount(dataRow.cash);
    totals.card += parseAmount(dataRow.card);
    totals.credit += parseAmount(dataRow.credit);
    totals.slip += parseAmount(dataRow.slip);
    totals.check += parseAmount(dataRow.check);
    totals.eWallet += parseAmount(dataRow.eWallet);
    totals.total += parseAmount(dataRow.total);
