...
/**
 * User Activity Report — Excel ONLY (A4 portrait, matches Print / PDF).
...
const HEADERS = ['No.', 'Date', 'User', 'Activity', 'Meta'] as const;

/** ~ PDF COL_PERCENTS [5, 14, 16, 42, 23] */
const COLUMN_WIDTHS = [5, 14, 16, 36, 20];
...
function dateRichText(date: string, time: string): string | ExcelJS.CellRichTextValue {
  if (!time) return date;
  return {
    richText: [
      { text: date, font: { bold: true, size: 8, name: 'Arial' } },
      { text: `\n${time}`, font: { size: 8, name: 'Arial', color: { argb: 'FF333333' } } },
    ],
  };
}

export type DownloadUserActivityExcelOptions = {
  reportName: string;
...
export async function downloadUserActivityReportExcel({
  reportName,
  summaryItems,
  generatedAt,
  rows,
...
}: DownloadUserActivityExcelOptions): Promise<void> {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (isNoteRow(r)) {
      ...
      noteCell.value = r.action;
      applyCellBase(noteCell, { italic: true, fill: true });
      for (let c = 2; c <= colCount; c++) {
        applyCellBase(sheet.getCell(row, c), { italic: true, fill: true });
      }
    ...
    const act = activityLines(r);
    const meta = metaLines(r);
    const values: Array<string | ExcelJS.CellRichTextValue> = [
      String(dataIndex),
      dateRichText(date, time),
      display(r.userName),
      labeledRichText(act),
      labeledRichText(meta),
    ];

    for (let c = 0; c < colCount; c++) {
      const cell = sheet.getCell(row, c + 1);
      cell.value = values[c]!;
      applyCellBase(cell, { horizontal: c === 0 ? 'center' : 'left' });
    }
    sheet.getRow(row).height = Math.max(42, Math.max(act.length, meta.length, time ? 2 : 1) * 14);
    row += 1;
  }
