...
/**
 * Channel Booking Details Report — Excel ONLY.
 * Grouped columns matching Print logical order; one booking per row.
 */
...
import {
  CHANNEL_BOOKINGS_EXPORT_GROUPS,
  CHANNEL_BOOKINGS_FEE_KEYS,
  channelBookingsExportCellValue,
  parseFeeTotal,
...
/** Sensible widths aligned with Print emphasis (Audit/Patient/Booking wider). */
const COLUMN_WIDTHS = [
  22, 14, // Doctor
  11, 14, 10, // Appointment
  16, 10, 10, // Booking
  18, 12, 12, // Patient
  12, 16, 14, // Refund
  24, 24, // Audit
  11, 11, 10, 11, // Fees / Total
  12, 14, // Payment
];

export type DownloadChannelBookingsReportExcelOptions = {
  reportName: string;
...
export async function downloadChannelBookingsReportExcel({
  reportName,
  summaryItems,
  generatedAt,
  rows,
  fileName = 'channel-bookings-report.xlsx',
  sheetName = 'Channel Bookings',
}: DownloadChannelBookingsReportExcelOptions): Promise<void> {
  const colCount = CHANNEL_BOOKINGS_EXPORT_GROUPS.reduce((n, g) => n + g.columns.length, 0);
  ...
  let colIndex = 1;
  for (const group of CHANNEL_BOOKINGS_EXPORT_GROUPS) {
  ...
  for (const group of CHANNEL_BOOKINGS_EXPORT_GROUPS) {
    for (const col of group.columns) {
      ...
      cell.alignment = {
        vertical: 'middle',
        horizontal: CHANNEL_BOOKINGS_FEE_KEYS.has(String(col.key)) ? 'right' : 'left',
        wrapText: true,
      };
      colIndex += 1;
    }
  ...
  for (const item of rows) {
    ...
    for (const group of CHANNEL_BOOKINGS_EXPORT_GROUPS) {
      for (const col of group.columns) {
        ...
        const value = channelBookingsExportCellValue(item, String(col.key));
        cell.value = value;
        ...
        const isFee = CHANNEL_BOOKINGS_FEE_KEYS.has(String(col.key));
        cell.alignment = {
          vertical: 'top',
  ...
  let totalFeeTotal = 0;
  for (const item of rows) {
    hospitalFeeTotal += parseFeeTotal(item, 'hospitalFee');
    doctorFeeTotal += parseFeeTotal(item, 'doctorFee');
    discountTotal += parseFeeTotal(item, 'discount');
    totalFeeTotal += parseFeeTotal(item, 'totalFee');
  }

  if (rows.length > 0) {
    let firstFeeCol = 1;
    for (const group of CHANNEL_BOOKINGS_EXPORT_GROUPS) {
      if (group.title === 'Fees / Total') break;
      firstFeeCol += group.columns.length;
    }

    const totalRow = sheet.getRow(row);
    if (firstFeeCol > 2) {
      sheet.mergeCells(row, 1, row, firstFeeCol - 1);
    }
    totalRow.getCell(1).value = 'Total';
    totalRow.getCell(1).font = { bold: true, size: 9, name: 'Arial' };

    const feeValues = [hospitalFeeTotal, doctorFeeTotal, discountTotal, totalFeeTotal];
    for (let i = 0; i < feeValues.length; i++) {
      const cell = totalRow.getCell(firstFeeCol + i);
      cell.value = feeValues[i];
      cell.numFmt = '#,##0.00';
      cell.font = { bold: true, size: 8, name: 'Arial' };
  ...
  // AutoFilter + freeze on field header row
