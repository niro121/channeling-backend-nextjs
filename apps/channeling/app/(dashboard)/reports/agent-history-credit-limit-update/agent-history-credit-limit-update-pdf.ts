...
/**
 * Agent History (Credit Limit Update) — PDF ONLY (A4 portrait, matches Print).
 */
...
import type { AgentHistoryCreditLimitUpdateReportExportRow } from '@/types/reports/agent-history-credit-limit-update';
import {
  AHCLU_PDF_COL_PERCENTS,
  AHCLU_PDF_HEADERS,
  ahcluPdfCompactRow,
  buildAhcluCompactRowsFromExport,
} from './agent-history-credit-limit-update-export-config';
...
export type DownloadAhcluPdfOptions = {
  reportName: string;
  summaryItems: BrandedPdfSummaryItem[];
  generatedAt: string;
...
export async function downloadAgentHistoryCreditLimitUpdateReportPdf({
  reportName,
  summaryItems,
  generatedAt,
  rows,
  fileName = 'agent-history-credit-limit-update.pdf',
}: DownloadAhcluPdfOptions): Promise<void> {
  const margin = 8;
  ...
  const compactRows = buildAhcluCompactRowsFromExport(rows);

  const columnStyles: Record<
    number,
    Partial<{ cellWidth: number; halign: 'left' | 'right' | 'center' }>
  > = {};
  AHCLU_PDF_COL_PERCENTS.forEach((pct, i) => {
    columnStyles[i] = {
  ...
  autoTable(doc, {
    head: [Array.from(AHCLU_PDF_HEADERS)],
    body: compactRows.map(ahcluPdfCompactRow),
    startY,
