...
import { AgentWiseAppointmentsPrintLayout } from './agent-wise-appointments-print-layout';
import { downloadAgentWiseAppointmentsReportPdf } from './agent-wise-appointments-pdf';
import { downloadAgentWiseAppointmentsReportExcel } from './agent-wise-appointments-excel';
...
export default function AgentWiseAppointmentsReportContent({
  ...
  const buildPrintSummaryItems = useCallback(
    (meta: NonNullable<typeof reportMeta>): ReportPrintSummaryItem[] => [
      {
        label: 'Period',
        value: formatReportRangeLabel(meta.from, meta.to),
        fullWidth: true,
      },
      ...
      {
        label: 'Report Type',
        value: meta.reportType === 'detail' ? 'Detail' : 'Summary',
      },
  ...
  return (
    <div className="container mx-auto py-3 space-y-4 agent-wise-appointments-report-root">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            ...
            <div className="no-print">
              <ExportWrapper<AgentWiseExportRow>
                ...
                customDownloadPdf={async ({ fileName }) => {
                  if (!reportMeta) {
                    throw new Error('Search to generate the report before exporting.');
                  }
                  const summaryItems = toBrandedPdfSummaryItems(
                    buildPrintSummaryItems(reportMeta)
                  );
                  const reportName = 'Agent Wise Appointments - Summary and Detail';
                  if (reportMeta.reportType === 'summary') {
                    await downloadAgentWiseAppointmentsReportPdf({
                      mode: 'summary',
                      reportName,
                      summaryItems,
                ...
                customDownloadExcel={async ({ fileName }) => {
                  if (!reportMeta) {
                    throw new Error('Search to generate the report before exporting.');
                  }
                  const summaryItems = toBrandedPdfSummaryItems(
                  ...
                  const reportName = 'Agent Wise Appointments - Summary and Detail';
                  if (reportMeta.reportType === 'summary') {
                    await downloadAgentWiseAppointmentsReportExcel({
                      mode: 'summary',
                      reportName,
                      summaryItems,
                      generatedAt: reportMeta.generatedAt,
                ...
                pdfSummaryItems={
                  reportMeta
                    ? toBrandedPdfSummaryItems(buildPrintSummaryItems(reportMeta))
                    : undefined
                }
                pdfGeneratedAt={reportMeta?.generatedAt}
              />
            </div>
          </div>
        </CardHeader>
        ...
        <Card className="bg-muted/20 print:shadow-none print:border-0 print:bg-white">
          ...
          <CardContent className="space-y-3 py-2">
            ...
            {loading ? (
              ...
              <>
            <div className="awa-screen-table print:hidden">
            ...
            <div className="awa-print-only hidden print:block">
              {reportMeta.reportType === 'summary' ? (
                <AgentWiseAppointmentsPrintLayout
                  mode="summary"
                  monthColumns={monthColumns}
                  summaryRows={summaryRows}
                  summaryMonthTotals={summaryMonthTotals}
                  summaryGrandTotal={summaryGrandTotal}
                  from={reportMeta.from}
                  to={reportMeta.to}
                />
              ) : (
                <AgentWiseAppointmentsPrintLayout
