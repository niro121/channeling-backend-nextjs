...
/**
 * Agent Wise Appointments — PDF ONLY (A4 portrait).
 * Summary: flat month table. Detail: compact categorized columns.
 * Same branded header for both report types.
 */
...
export type DownloadAwaSummaryPdfOptions = CommonOpts & {
  mode: 'summary';
...
export type DownloadAwaDetailPdfOptions = CommonOpts & {
  mode: 'detail';
...
export type DownloadAgentWiseAppointmentsPdfOptions =
  | DownloadAwaSummaryPdfOptions
  | DownloadAwaDetailPdfOptions;

export async function downloadAgentWiseAppointmentsReportPdf(
  opts: DownloadAgentWiseAppointmentsPdfOptions
): Promise<void> {
  const margin = 8;
  ...
  if (opts.mode === 'summary') {
    ...
    const percents = buildAwaSummaryColPercents(opts.monthColumns.length);
    ...
    autoTable(doc, {
      ...
      styles: {
        font: 'helvetica',
        fontSize: opts.monthColumns.length > 6 ? 5.5 : 7,
      ...
      headStyles: {
        ...
        textColor: [0, 0, 0],
        fontStyle: 'bold',
        fontSize: opts.monthColumns.length > 6 ? 5 : 6.25,
        valign: 'middle',
        lineColor: [0, 0, 0],
        lineWidth: 0.2,
      },
    ...
    const columnStyles: Record<
      ...
      Partial<{ cellWidth: number; halign: 'left' | 'right' | 'center' }>
    > = {};
    AWA_DETAIL_PDF_COL_PERCENTS.forEach((pct, i) => {
      columnStyles[i] = {
        cellWidth: (tableWidth * pct) / 100,
    ...
    autoTable(doc, {
      head: [Array.from(AWA_DETAIL_PDF_HEADERS)],
      body: compactRows.map(awaDetailPdfCompactRow),
      startY,
      margin: { left: margin, right: margin, bottom: 12 },
      tableWidth,
      showHead: 'everyPage',
  ...
  doc.save(opts.fileName ?? 'agent-wise-appointments-report.pdf');
}
