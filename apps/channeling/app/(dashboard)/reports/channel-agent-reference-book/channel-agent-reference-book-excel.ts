...
/**
 * Channel Agent Reference Book — Excel ONLY (A4 portrait, matches Print).
 */
...
const COL_COUNT = 7;
/** Rough match to print colgroup proportions. */
const COLUMN_WIDTHS = [8, 14, 18, 10, 20, 20, 10];
...
export type DownloadChannelAgentReferenceBookExcelOptions = {
  reportName: string;
...
export async function downloadChannelAgentReferenceBookReportExcel({
  reportName,
  summaryItems,
  generatedAt,
  rows,
  fileName = 'channel-agent-reference-book-report.xlsx',
  sheetName = 'Agent Reference Book',
}: DownloadChannelAgentReferenceBookExcelOptions): Promise<void> {
  ...
  const headers = Array.from(CHANNEL_AGENT_REF_PDF_HEADERS);
  ...
  for (const [groupTitle, groupRows] of groups.entries()) {
    ...
    for (const item of groupRows) {
      const compact = mapChannelAgentReferenceBookCompactFromExportRow(item);
      const values = [
        compact.sNo,
        compact.bookNumber,
        `Start: ${compact.startRef}\nEnd: ${compact.endRef}`,
        compact.pages,
        `${compact.createdBy}\n${compact.createdAt}`,
        `${compact.updatedBy}\n${compact.updatedAt}`,
        compact.status,
