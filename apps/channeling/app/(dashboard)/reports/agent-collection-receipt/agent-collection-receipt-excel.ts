...
/**
 * Agent Collection Receipt — Excel ONLY (A4 portrait, matches Print / PDF).
 */
...
const COL_COUNT = 6;
/** Proportional to ACR_PDF_COL_PERCENTS. */
const COLUMN_WIDTHS = [5, 16, 18, 18, 26, 28];
...
export type DownloadAcrExcelOptions = {
  reportName: string;
...
export async function downloadAgentCollectionReceiptReportExcel({
  reportName,
  summaryItems,
  generatedAt,
  rows,
  fileName = 'agent-collection-receipt-report.xlsx',
  sheetName = 'Collection Receipts',
}: DownloadAcrExcelOptions): Promise<void> {
  ...
  const headers = Array.from(ACR_PDF_HEADERS);
  ...
  for (const compact of compactRows) {
    const values = acrPdfCompactRow(compact);
    const isTotal = Boolean(compact.isTotal);
    const isNeg = Boolean(compact.isNegative);
    ...
    for (let c = 0; c < colCount; c++) {
      ...
      cell.font = {
        ...
        name: 'Arial',
        bold: isTotal,
        color: isNeg ? { argb: 'FFB91C1C' } : undefined,
      };
      cell.border = thinBorder;
      cell.alignment = {
        vertical: 'top',
        horizontal: isTotal ? (c === 4 ? 'left' : 'left') : c === 0 ? 'center' : 'left',
        wrapText: true,
      };
      if (isTotal) {
    ...
    sheet.getRow(row).height = isTotal ? 56 : 58;
    row += 1;
  }
