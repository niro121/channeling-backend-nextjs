...
/**
 * Card Summary - Bank Wise — Excel ONLY (A4 portrait, matches Print / PDF).
...
const SUMMARY_HEADERS = ['No.', 'Bank Name', 'Count', 'Total'] as const;
const DETAIL_HEADERS = ['No.', 'Receipt', 'Details', 'Bank', 'Card No', 'Total'] as const;

/** ~ SUMMARY_COL_PERCENTS [8, 52, 18, 22] */
const SUMMARY_WIDTHS = [6, 42, 12, 14];
/** ~ DETAIL_COL_PERCENTS [5, 14, 36, 16, 14, 15] */
const DETAIL_WIDTHS = [5, 14, 36, 14, 12, 12];
...
/** Bold labels + normal values, matching print / PDF. */
...
export type DownloadCardSummaryBankWiseExcelOptions = {
  reportName: string;
...
export async function downloadCardSummaryBankWiseReportExcel({
  reportName,
  summaryItems,
  generatedAt,
  format,
  rows,
  fileName = 'card-summary-bank-wise.xlsx',
  sheetName = 'Card Summary',
}: DownloadCardSummaryBankWiseExcelOptions): Promise<void> {
  const isDetail = format === 'detail';
  const headers = isDetail ? DETAIL_HEADERS : SUMMARY_HEADERS;
  const widths = isDetail ? DETAIL_WIDTHS : SUMMARY_WIDTHS;
  ...
  for (let c = 0; c < headers.length; c++) {
    ...
    if (c === 0) horizontal = 'center';
    else if (!isDetail && (c === 2 || c === 3)) horizontal = 'right';
    else if (isDetail && c === 5) horizontal = 'right';
