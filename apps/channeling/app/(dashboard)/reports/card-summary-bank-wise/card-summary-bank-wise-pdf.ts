...
/**
 * Card Summary - Bank Wise — PDF ONLY (A4 portrait, matches Print).
 * Summary: No. | Bank Name | Count | Total
 * Detail: No. | Receipt | Details | Bank | Card No | Total
 */
...
const SUMMARY_COL_PERCENTS = [8, 52, 18, 22] as const;
const DETAIL_COL_PERCENTS = [5, 14, 36, 16, 14, 15] as const;
...
async function drawHeader(
  ...
  // Period (fullWidth) on its own row, then remaining items in up to 3 columns.
...
function getDetailLines(row: CardSummaryBankWiseReportExportRow): DetailLine[] {
...
/** Plain text used only so autoTable can measure cell height. */
function detailsHeightText(lines: DetailLine[]): string {
  return lines.map((l) => `${l.label} ${l.value}`).join('\n');
}

type CellWithDetailLines = { csbwDetailLines?: DetailLine[] };

export type DownloadCardSummaryBankWisePdfOptions = {
  reportName: string;
  summaryItems: BrandedPdfSummaryItem[];
  generatedAt: string;
  format: CardSummaryBankWiseReportFormat;
  rows: CardSummaryBankWiseReportExportRow[];
  fileName?: string;
};

export async function downloadCardSummaryBankWiseReportPdf({
  reportName,
  summaryItems,
  generatedAt,
  format,
  ...
  const colPercents = format === 'detail' ? DETAIL_COL_PERCENTS : SUMMARY_COL_PERCENTS;
  const columnStyles: Record<
    number,
    Partial<{ cellWidth: number; halign: 'left' | 'right' | 'center' }>
  > = {};
  colPercents.forEach((pct, i) => {
    let halign: 'left' | 'right' | 'center' = 'left';
    if (i === 0) halign = 'center';
    else if (format === 'summary' && (i === 2 || i === 3)) halign = 'right';
    else if (format === 'detail' && i === 5) halign = 'right';
    columnStyles[i] = {
  ...
  if (format === 'summary') {
    head = [['No.', 'Bank Name', 'Count', 'Total']];
    body = [
      ...rows.map((r, i) => [String(i + 1), r.bank || '—', r.count || '0', r.total || '0.00']),
      ['', 'Total', String(totalCount), formatAmount(totalAmount)],
    ];
  } else {
    head = [['No.', 'Receipt', 'Details', 'Bank', 'Card No', 'Total']];
    body = [
      ...rows.map((r, i) => [
        String(i + 1),
        r.receiptNo || '—',
  ...
  autoTable(doc, {
    ...
    didParseCell: (hook) => {
      ...
      if (format === 'detail' && hook.column.index === 2) {
        const row = rows[hook.row.index];
        if (row) {
          (hook.cell as typeof hook.cell & CellWithDetailLines).csbwDetailLines =
            getDetailLines(row);
        }
      }
    },
    willDrawCell: (hook) => {
      if ((hook.cell as typeof hook.cell & CellWithDetailLines).csbwDetailLines) {
        hook.cell.text = [];
      }
    },
    didDrawCell: (hook) => {
      const lines = (hook.cell as typeof hook.cell & CellWithDetailLines).csbwDetailLines;
