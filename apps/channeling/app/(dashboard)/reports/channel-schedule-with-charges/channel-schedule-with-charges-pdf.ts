...
/**
 * Channel Schedule with Charges — PDF ONLY (A4 portrait, matches Print).
 */
...
import {
  CHANNEL_SCHEDULE_PDF_COL_PERCENTS,
  CHANNEL_SCHEDULE_PDF_HEADERS,
  channelScheduleDoctorGroupTitle,
  channelSchedulePdfCompactRow,
...
export type DownloadChannelScheduleWithChargesPdfOptions = {
  reportName: string;
  summaryItems: BrandedPdfSummaryItem[];
  generatedAt: string;
...
export async function downloadChannelScheduleWithChargesReportPdf({
  reportName,
  summaryItems,
  generatedAt,
  rows,
  fileName = 'channel-schedule-with-charges.pdf',
}: DownloadChannelScheduleWithChargesPdfOptions): Promise<void> {
  const margin = 7;
  ...
  const groups = new Map<string, ChannelScheduleWithChargesReportExportRow[]>();
  for (const row of rows) {
    const key = channelScheduleDoctorGroupTitle(row);
    if (!groups.has(key)) groups.set(key, []);
    groups.get(key)!.push(row);
  }

  const columnStyles: Record<number, { cellWidth: number }> = {};
  CHANNEL_SCHEDULE_PDF_COL_PERCENTS.forEach((pct, i) => {
  ...
  for (const [groupTitle, groupRows] of groups.entries()) {
    ...
    autoTable(doc, {
      head: [Array.from(CHANNEL_SCHEDULE_PDF_HEADERS)],
      body: groupRows.map((row) => channelSchedulePdfCompactRow(row)),
      startY: y,
