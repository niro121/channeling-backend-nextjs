...
/**
 * Agent Balance Report — Excel ONLY (A4 portrait, matches Print / PDF).
 */
...
import {
  AGENT_BALANCE_PDF_HEADERS,
  agentBalancePdfCompactRow,
  buildAgentBalanceCompactRows,
} from './agent-balance-export-config';
...
const COL_COUNT = 6;
/** Proportional to AGENT_BALANCE_PDF_COL_PERCENTS. */
const COLUMN_WIDTHS = [6, 9, 28, 32, 24, 14];
...
export type DownloadAgentBalanceExcelOptions = {
  reportName: string;
...
export async function downloadAgentBalanceReportExcel({
  reportName,
  summaryItems,
  generatedAt,
  rows,
  balanceTotal,
  fileName = 'agent-balance-report.xlsx',
  sheetName = 'Agent Balance',
}: DownloadAgentBalanceExcelOptions): Promise<void> {
  ...
  const headers = Array.from(AGENT_BALANCE_PDF_HEADERS);
  for (let c = 0; c < headers.length; c++) {
    ...
    cell.alignment = {
      vertical: 'middle',
      horizontal: c === 0 || c === 1 ? 'center' : c === 5 ? 'right' : 'left',
      wrapText: true,
    };
  }
  ...
  const compactRows = buildAgentBalanceCompactRows(rows, balanceTotal);
  for (const compact of compactRows) {
    const values = agentBalancePdfCompactRow(compact);
    ...
    sheet.getRow(row).height = isTotal ? 18 : 42;
    row += 1;
  }
