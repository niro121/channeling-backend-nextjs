...
/**
 * Agent Balance Report — PDF ONLY (A4 portrait, matches Print).
 */
...
import {
  AGENT_BALANCE_PDF_COL_PERCENTS,
...
export type DownloadAgentBalancePdfOptions = {
  reportName: string;
  summaryItems: BrandedPdfSummaryItem[];
  generatedAt: string;
...
export async function downloadAgentBalanceReportPdf({
  reportName,
  summaryItems,
  generatedAt,
  rows,
  balanceTotal,
  fileName = 'agent-balance-report.pdf',
}: DownloadAgentBalancePdfOptions): Promise<void> {
  const margin = 8;
  ...
  const columnStyles: Record<number, Partial<{ cellWidth: number; halign: 'left' | 'right' | 'center' }>> =
    {};
  AGENT_BALANCE_PDF_COL_PERCENTS.forEach((pct, i) => {
    columnStyles[i] = {
      cellWidth: (tableWidth * pct) / 100,
      halign: i === 0 || i === 1 ? 'center' : i === 5 ? 'right' : 'left',
    };
  });
