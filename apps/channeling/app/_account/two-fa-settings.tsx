...
import { twoFADispatchStatusChanged } from '@/app/(dashboard)/welcome/two-fa-banner';

type SetupStep = 'idle' | 'show-qr' | 'verify';

export function TwoFASettings() {
  const { toast } = useToast();
  const [hasAuthenticator, setHasAuthenticator] = useState<boolean | null>(
    null
  );
  ...
  const handleStartSetup = async () => {
    ...
    try {
      ...
      toast({
        variant: 'destructive',
        title: 'Error',
        description: e.message ?? 'Could not start setup'
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          'Authenticator app is set up. You can use it at login when you have 2FA enabled.'
      });
      setSetupStep('idle');
      setSetupData(null);
      setVerifyCode('');
      ...
      toast({
        variant: 'destructive',
        title: 'Error',
        description: e.message ?? 'Invalid code'
      });
    } finally {
      setVerifying(false);
    }
  ...
  const handleRequire2FAToggle = async (checked: boolean) => {
    ...
    try {
      ...
      toast({
        variant: 'destructive',
        title: 'Error',
        description: e.message ?? 'Could not update preference'
      });
    } finally {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            When turned on, you will be asked for a verification code
            (authenticator app or SMS) when you sign in. Turn this on only if
            you have set up or are ready to set up an authenticator or other
            method. (Email verification is currently unavailable.)
          </CardDescription>
        </CardHeader>
          ...
          {!groupAllows2FA && (
            <div className="rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-900/50 dark:bg-amber-950/30 p-4">
              ...
              <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                Your administrator has turned off two-factor authentication for
                your group. At login you will not be asked for a code until they
                enable 2FA for your group. You can still turn on the switch
                below; your preference will apply as soon as your group allows
                2FA.
              </p>
            </div>
          )}
          ...
          <div className="flex items-center justify-between rounded-lg border p-4">
            <div>
              <p className="font-medium">
                Ask for verification code when I sign in
              </p>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Use an authenticator app (e.g. Google Authenticator, Authy) to get a
            verification code when you have 2FA enabled at login.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
            ...
            <div className="flex items-center gap-3 rounded-lg border border-green-200 bg-green-50 dark:border-green-900/50 dark:bg-green-950/30 p-4">
              ...
              <div>
                ...
                <p className="text-sm text-green-700 dark:text-green-300">
                  You will be asked for a code from your app when you sign in
                  with 2FA enabled.
                </p>
              </div>
            </div>
          ...
          {!hasAuthenticator && setupStep === 'idle' && (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Set up an authenticator app so you can use it as a 2FA method at
                login. You will scan a QR code or enter a code into your app.
              </p>
          ...
          {setupStep === 'show-qr' && setupData && (
            <div className="space-y-6">
              ...
              <p className="text-sm text-muted-foreground">
                After adding the account in your app, click below and enter the
                6-digit code to confirm.
              </p>
              <div className="flex gap-2">
                <Button onClick={() => setSetupStep('verify')}>
                  I&apos;ve added the app — verify
                </Button>
                <Button
                  type="button"
                  variant="ghost"
