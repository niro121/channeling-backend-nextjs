...
const DOCTOR_PAYMENT_HANDED_PREFIX = 'DOCTOR PAYMENT Handed:';

function parseHandedToStaff(remarks?: string | null): string {
  const text = remarks?.trim() ?? '';
  if (!text.startsWith(DOCTOR_PAYMENT_HANDED_PREFIX)) return '-';
  const name = text.slice(DOCTOR_PAYMENT_HANDED_PREFIX.length).trim();
  return name || '-';
}
  ...
  try {
    ...
    const rows: ChannelReportReceiptWiseRow[] = receipts.map((receipt) => {
      ...
      return {
        ...
        handedToStaff: parseHandedToStaff(receipt.remarks),
        cancelReason:
