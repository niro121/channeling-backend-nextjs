...
type PaymentMethodFieldsProps = {
  ...
  disabled?: boolean;
  /** Restrict selectable methods (e.g. refund cash-only / original-or-cash). */
  allowedMethods?: PatientBillPaymentMethod[];
  /** Fieldset label — defaults to "Payment method". */
  methodLabel?: string;
...
export function emptyPaymentMethodValues(
...
/**
 * Shared payment-method UI used by patient bill record payment, doctor payout,
 * and (via RefundMethodFields) receipt/doctor refunds.
 */
export function PaymentMethodFields({
  ...
  useEffect(() => {
    ...
    if (fallback != null) {
      onChange(emptyPaymentMethodValues(fallback));
    }
  ...
  const patch = (partial: Partial<PaymentMethodValues>) => {
  ...
  return (
    <div className="space-y-3">
      <div className="space-y-1">
        <Label className="text-xs font-medium">{methodLabel}</Label>
        <Select
          ...
          onValueChange={(nextValue) => {
            ...
            onChange(emptyPaymentMethodValues(next));
          }}
        ...
        >
          ...
          >
            <SelectValue placeholder={`Select ${methodLabel.toLowerCase()}`} />
          </SelectTrigger>
          <SelectContent>
        ...
        ) : allowedMethods && methodOptions.length > 1 ? (
          <p className="text-[11px] text-muted-foreground">
      ...
      {showCard ? (
        <div className="grid grid-cols-2 gap-2.5">
          <div className="space-y-1">
            <Label htmlFor="pay-card-last4" className="text-xs font-medium">
              Last 4 Digits
            </Label>
            <Input
              id="pay-card-last4"
      ...
      {showSlip || showCheque ? (
        <div className="grid grid-cols-3 gap-2.5">
          <div className="space-y-1">
            <Label htmlFor="pay-slip-ref" className="text-xs font-medium">
          ...
          <div className="space-y-1">
            <Label htmlFor="pay-slip-date" className="text-xs font-medium">
      ...
      {showEWallet ? (
        <div className="space-y-1">
          <Label htmlFor="pay-ewallet-ref" className="text-xs font-medium">
            E-Wallet Reference
          </Label>
          <Input
            id="pay-ewallet-ref"
