...
export default async function DoctorPaymentReportPage({
  searchParams,
}: SearchParams) {
  ...
  const result = await getDoctorPaymentReportAction({
    page,
    limit,
    keyword,
    dateFrom,
  ...
  return (
    <div className="space-y-6 overflow-hidden">
      <div className="space-y-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Doctor Payment Report</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Doctor payment receipts with search, print, and export.
          </p>
        </div>
      </div>
      ...
      <Suspense fallback={<Loading />}>
        <ReportsTable
          ...
          toolbarLeft={
            <div className="flex flex-wrap items-center gap-3">
              <div className="relative w-full sm:max-w-sm">
                <SearchInput
                  name="keyword"
                  placeholder="Search by doctor or receipt number..."
          ...
          toolbarRight={
            <ReportsToolbar variant="doctor-payments" serverData={handleExport} />
          }
        />
      </Suspense>
    </div>
