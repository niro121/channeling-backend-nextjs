...
export function TwoFASettings() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      toast({ title: 'Success', style: { backgroundColor: '#f0fdf4' }, description: 'Authenticator app is set up.' });
      setSetupStep('idle');
      setSetupData(null);
      setVerifyCode('');
      fetchStatus();
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            When turned on, you will be asked for a verification code (authenticator app or SMS) when you sign in.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {!groupAllows2FA && (
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-4">
              <p className="font-medium text-amber-800">2FA is disabled for your group</p>
              <p className="text-sm text-amber-700 mt-1">
                Your administrator has turned off two-factor authentication for your group.
              </p>
            </div>
          )}
          {!hasPhone && (
            <div className="rounded-lg border border-red-200 bg-red-50 p-4">
              <p className="font-medium text-red-600">Phone number required</p>
              <p className="text-sm text-red-700 mt-1">
                Add a mobile number to your profile to enable 2FA.
              </p>
            </div>
          )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Use an authenticator app (e.g. Google Authenticator, Authy) to get a verification code at login.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {hasAuthenticator && setupStep === 'idle' && (
            <div className="flex items-center gap-3 rounded-lg border border-green-200 bg-green-50 p-4">
              ...
              <div>
                <p className="font-medium text-green-800">Authenticator app is set up</p>
                <p className="text-sm text-green-700">You will be asked for a code when you sign in with 2FA enabled.</p>
              </div>
            </div>
          )}
          ...
          {!hasAuthenticator && setupStep === 'idle' && (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Set up an authenticator app to use it as a 2FA method at login.
              </p>
          ...
          {setupStep === 'show-qr' && setupData && (
            <div className="space-y-6">
              ...
              <div className="flex gap-2">
                ...
                <Button type="button" variant="ghost" onClick={() => { setSetupStep('idle'); setSetupData(null); }}>
                  Cancel
                </Button>
              </div>
            </div>
          ...
          {setupStep === 'verify' && (
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <Button type="button" variant="ghost" onClick={() => setSetupStep('show-qr')}>Back</Button>
              </div>
            </div>
          )}
        </CardContent>
