...
const APP_NAME = process.env.NEXT_PUBLIC_APP_NAME || 'Archmage DPAY';
...
export async function POST(request: Request) {
  try {
    ...
    if (!groupAllows2FA) return NextResponse.json({ error: '2FA is disabled for your group.' }, { status: 403 });

    const allowedMethods = Array.isArray(group?.twoFactorMethods) && group.twoFactorMethods.length > 0 ? group.twoFactorMethods : ['1', '2', '3'];
    if (!allowedMethods.includes(method)) return NextResponse.json({ error: 'Method not allowed' }, { status: 403 });

    const expiresMinutes = method === TWO_FACTOR_METHODS.AUTH_APP ? TWO_FA_PENDING_EXPIRY_MINUTES : TWO_FA_CODE_EXPIRY_MINUTES;
    ...
    if (method === TWO_FACTOR_METHODS.AUTH_APP) {
      ...
      if (!user.twoFactorSecret) {
        ...
        await authPrisma.user.update({ where: { id: user.id }, data: { twoFactorMethod: TWO_FACTOR_METHODS.AUTH_APP, twoFactorPendingSecret: secret, twoFactorTempCode: token, twoFactorExpires: expiresAt } });
      ...
      await authPrisma.user.update({ where: { id: user.id }, data: { twoFactorTempCode: token, twoFactorExpires: expiresAt, twoFactorPendingSecret: null } });
    ...
    if (method === TWO_FACTOR_METHODS.SMS) {
      ...
      if (!phone) return NextResponse.json({ error: 'Phone number not set.' }, { status: 400 });
      const code = generateSixDigitCode();
      const smsResult = await send2faSms(phone, code);
      if (!smsResult.success) return NextResponse.json({ error: 'Failed to send SMS. Try another method.' }, { status: 503 });
      await authPrisma.user.update({ where: { id: user.id }, data: { twoFactorTempCode: code, twoFactorExpires: expiresAt, twoFactorPendingSecret: null } });
    ...
    if (method === TWO_FACTOR_METHODS.EMAIL) {
      ...
      if (!emailResult.success) return NextResponse.json({ error: 'Email verification is currently unavailable.' }, { status: 503 });
    }
