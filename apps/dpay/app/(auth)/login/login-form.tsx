...
import { FirstLoginPasswordDialog } from './first-login-password-dialog';
...
const LoginForm = () => {
  ...
  const initialValues: FormValues = { email: '', password: '', twoFactorCode: '', invalidCredentials: '' };
  ...
  const handleSubmit = async (values: FormValues, { setErrors }: FormikHelpers<FormValues>) => {
    try {
      if (pending2FA?.selectedMethod) {
        ...
        const result = await signIn('credentials', { redirect: false, username: pending2FA.email, password: pending2FA.password, twoFactorCode: values.twoFactorCode.trim(), twoFactorToken: useToken });
        if (result?.error) { setErrors({ invalidCredentials: 'Invalid or expired code. Try again or choose another method.' }); return; }
        setIsRedirecting(true);
        router.replace('/welcome');
        return;
      }

      const checkRes = await fetch('/api/auth/check-login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: values.email, password: values.password }) });
      const checkData = await checkRes.json().catch(() => ({}));
      if (checkRes.status === 401) { setErrors({ invalidCredentials: 'Invalid credentials' }); return; }
      if (checkRes.status !== 200) { setErrors({ invalidCredentials: checkData?.error ?? 'Something went wrong' }); return; }
      if (checkData.requiresPasswordChange === true) { setPasswordChangeEmail(values.email); setErrors({ invalidCredentials: '' }); return; }
      if (checkData.requiresTwoFactor === true && Array.isArray(checkData.allowedMethods) && checkData.allowedMethods.length > 0) {
        setPending2FA({ email: values.email, password: values.password, allowedMethods: checkData.allowedMethods });
        setErrors({ invalidCredentials: '' });
        return;
      }

      const result = await signIn('credentials', { redirect: false, username: values.email, password: values.password });
      if (result?.error) { setErrors({ invalidCredentials: 'Invalid credentials' }); return; }
      setIsRedirecting(true);
      router.replace('/welcome');
    } catch (error: any) {
  ...
  const handleSelectMethod = async (methodId: string, clearTwoFactorCode: () => void) => {
    if (!pending2FA || requestingCode) return;
    clearTwoFactorCode();
    setRequestingCode(true);
    try {
      const res = await fetch('/api/auth/request-2fa-code', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: pending2FA.email, password: pending2FA.password, method: methodId }) });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) { toast({ variant: 'destructive', title: 'Error', description: data?.error ?? 'Could not send code.' }); return; }
      setPending2FA((prev) => prev ? { ...prev, selectedMethod: methodId, twoFactorToken: data.twoFactorToken ?? null, message: data.message ?? 'Enter your verification code below.', needsSetup: !!data.needsSetup, uri: data.uri, secret: data.secret } : null);
  ...
  const handleBackFrom2FA = (clearTwoFactorCode: () => void) => {
    clearTwoFactorCode();
    if (pending2FA?.selectedMethod) {
      setPending2FA((prev) => prev ? { ...prev, selectedMethod: undefined, twoFactorToken: undefined, message: undefined, needsSetup: undefined, uri: undefined, secret: undefined } : null);
    } else {
      setPending2FA(null);
    }
  };
    ...
    <>
      <FirstLoginPasswordDialog
        ...
        onSuccess={async (email, newPassword) => {
          ...
          const result = await signIn('credentials', { redirect: false, username: email, password: newPassword });
          if (result?.error) { toast({ variant: 'destructive', title: 'Error', description: 'Invalid credentials' }); return; }
          setIsRedirecting(true);
          router.replace('/welcome');
        }}
      />
      <div className="lg:hidden mb-6 text-center">
        <span className="text-xl font-semibold text-foreground">{process.env.NEXT_PUBLIC_BRAND_NAME || 'Ruhunu'} DPAY</span>
      </div>
      <Card className="w-full border-0 shadow-none bg-transparent p-0">
        <CardHeader className="space-y-1 px-0 pt-0">
          <CardTitle className="text-2xl">
            {pending2FA ? pending2FA.selectedMethod ? (pending2FA.needsSetup ? 'Set up authenticator app' : 'Enter verification code') : 'Choose verification method' : 'Login'}
          </CardTitle>
          <CardDescription>
            {pending2FA?.selectedMethod ? (pending2FA.needsSetup ? 'Scan the QR code or enter the secret in your app, then enter the 6-digit code below.' : (pending2FA.message ?? 'Enter the code you received below.')) : pending2FA ? 'Select how you want to receive your verification code.' : 'Enter your credentials to sign in'}
          </CardDescription>
        </CardHeader>
                      ...
                      <>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex items-center gap-2">
                            <code className="flex-1 rounded-md border bg-muted/50 px-3 py-2 text-sm font-mono tracking-wider break-all">{formatSecret(pending2FA.secret)}</code>
                            <Button type="button" variant="outline" size="icon" onClick={() => { if (pending2FA.secret) { navigator.clipboard.writeText(pending2FA.secret); setCopied(true); setTimeout(() => setCopied(false), 2000); } }}>
                    ...
                    <div className="grid gap-2">
                      {allowedMethodOptions.map((method) => (
                        <Button key={method.id} type="button" variant="outline" className="h-auto justify-start gap-3 py-3 px-4 text-left" onClick={() => handleSelectMethod(method.id, () => { formik.setFieldValue('twoFactorCode', ''); formik.setFieldError('invalidCredentials', undefined); })} disabled={requestingCode}>
                          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md border bg-muted">{METHOD_ICONS[method.id] ?? <Smartphone className="h-5 w-5" />}</span>
                          <span className="font-medium">{method.option}</span>
                          {requestingCode && <span className="ml-auto text-muted-foreground text-sm">Sending…</span>}
                        </Button>
                      ))}
                    </div>
                  </div>
