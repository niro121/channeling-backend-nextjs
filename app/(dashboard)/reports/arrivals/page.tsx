...
function DoctorArrivalsReportContent() {
    ...
    const loadOptions = async () => {
      ...
      try {
        ...
        if (doctorsResult.success && doctorsResult.data) {
          // Format doctors with helper function
          const formattedDoctors = doctorsResult.data
            .filter((doctor: any) => doctor.id) // Filter out doctors without id
            .map((doctor: any) => ({
              id: doctor.id || '',
              name: formatDoctorName(doctor)
            }));
          // Add "All Doctors" option
          setDoctorOptions([{ id: '__all__', name: 'All Doctors' }, ...formattedDoctors]);
        }

        if (locationsResult.success && locationsResult.data) {
          const formattedLocations = locationsResult.data.map((loc: any) => ({
            id: loc.id || '',
            name: loc.name || ''
          }));
          // Add "All Branches" option
          setLocationOptions([{ id: '__all__', name: '' }, ...formattedLocations]);
        }
      } catch (error) {
...
export default function DoctorArrivalsReportPage() {
  return (
