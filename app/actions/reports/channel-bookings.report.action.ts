  ...
  try {
    ...
    const mapped: ChannelBookingsReportExportRow[] = result.data.map((row: any) => {
      ...
      return {
        ...
        updater,
        creator,
        hospitalFee: typeof row.hospitalFee === 'number' ? (row.hospitalFee / 100).toFixed(2) : '-',
        doctorFee: typeof row.professionalFee === 'number' ? (row.professionalFee / 100).toFixed(2) : '-',
        discount: typeof row.discount === 'number' ? (row.discount / 100).toFixed(2) : '-',
        totalFee: typeof row.amount === 'number' ? (row.amount / 100).toFixed(2) : '-',
