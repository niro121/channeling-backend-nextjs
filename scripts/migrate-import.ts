 ...
 *
 * Steps for --only (comma-separated): specialities, doctors, departments, locations, zones, rooms, tags, discounts, agencies.
...
async function migrateFetch<T>(
  ...
  if (data.error_code !== 0) {
    if (data.error_code === 1) throw new Error('Invalid or missing user_key');
    throw new Error(`API error_code ${data.error_code}`);
  }
  const list = data[listKey];
  return Array.isArray(list) ? list : [];
}

const ALL_STEPS = ['specialities', 'doctors', 'departments', 'locations', 'zones', 'rooms', 'tags', 'discounts', 'agencies'] as const;
type StepName = (typeof ALL_STEPS)[number];

function parseArgs(): { flush: boolean | null; only: StepName[] | null } {
...
async function importSpecialities(importUserId: string): Promise<Map<string, string>> {
  const list = await migrateFetch<SourceSpeciality>('all-specialties', 'specialitylist');
  const map = new Map<string, string>();
  for (const s of list) {
...
async function importDoctors(specialityIdMap: Map<string, string>, importUserId: string): Promise<void> {
  ...
  for (const d of list) {
    ...
    if (!specialityId) {
      console.warn(`  Skip doctor ${d.id} (${d.name}): specialty ${d.speciality_id} not found`);
      skipped++;
      continue;
    }
    try {
      ...
      if (e?.code === 'P2002') {
        ...
        console.warn(`  Skip doctor ${d.id} (${d.name}): duplicate ${target} (e.g. mobile/code/registrationNumber)`);
        skipped++;
      } else {
        throw e;
      }
...
async function importLocations(importUserId: string): Promise<Map<string, string>> {
...
async function importZonesFromApi(
  ...
  const list = await migrateFetch<SourceZone>('all-zones', 'zonelist');
  const zoneIdMap = new Map<string, string>();
  let skipped = 0;
  for (const z of list) {
    const locationId = locationIdMap.get(z.location);
    if (!locationId) {
      console.warn(`  Skip zone ${z.id} (${z.name}): location ${z.location} not found`);
      skipped++;
      continue;
    }
...
async function importDiscounts(importUserId: string): Promise<void> {
...
async function importAgencies(importUserId: string): Promise<Map<string, string>> {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (stepsToRun.includes('specialities')) {
      ...
      specialityIdMap = await importSpecialities(importUserId);
    ...
    if (stepsToRun.includes('doctors')) {
      ...
      await importDoctors(specialityIdMap, importUserId);
    }
    ...
    if (stepsToRun.includes('locations')) {
      ...
      locationIdMap = await importLocations(importUserId);
    }
    ...
    if (stepsToRun.includes('zones')) {
      ...
      zoneIdMap = await importZonesFromApi(locationIdMap, importUserId);
      if (zoneIdMap.size === 0) {
    ...
    if (stepsToRun.includes('rooms')) {
      ...
      await importRooms(locationIdMap, zoneIdMap, zoneByLocationMap, importUserId);
    }

    if (stepsToRun.includes('tags')) {
      console.log('[Step] Tags');
      await importTags(importUserId);
    }

    if (stepsToRun.includes('discounts')) {
      console.log('[Step] Discounts');
      await importDiscounts(importUserId);
    }

    if (stepsToRun.includes('agencies')) {
      console.log('[Step] Agencies');
      await importAgencies(importUserId);
    }
  } catch (e) {
